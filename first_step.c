...
// PROGS
void A_STAR(int x, int y, int w, int h, int STEP);	  		// Алгоритм поиска пути А*
...
//////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////
void A_STAR(int x, int y, int w, int h, int STEP)
{
	...
	if (VALID_DOT(x, y, w, h))
	{
		if ((DOTS[GET_NUM(x, y, w)].mass == -1) || (DOTS[GET_NUM(x, y, w)].mass > STEP))
		{
			...
			MOVES[x][y] = DOTS[GET_NUM(x, y, w)].mass;
			DOTS[GET_NUM(x, y, w)].num = GET_NUM(x, y, w);
			A_STAR(x, y, w, h, STEP);
		}
		else
		{
			if (VALID_DOT(x + 1, y, w, h)) 	A_STAR(x + 1, y, w, h, STEP + 1);
			if (VALID_DOT(x - 1, y, w, h)) 	A_STAR(x - 1, y, w, h, STEP + 1);
			if (VALID_DOT(x, y + 1, w, h))	A_STAR(x, y + 1, w, h, STEP + 1);
			if (VALID_DOT(x, y - 1, w, h))	A_STAR(x, y - 1, w, h, STEP + 1);
		}
	}
	else return;
